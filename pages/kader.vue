<template>
  <div class="p-4">
    <h2 class="text-xl font-bold mt-4">Kader</h2>

    <!-- Input + Hinzufügen -->
    <div class="flex flex-row gap-2 mt-4">
      <input
        type="text"
        v-model="spielerInput"
        placeholder="Vorname Nachname"
        class="flex-1 p-2 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
      />
      <button
        @click="addSpieler"
        class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-400 transition-all duration-200"
      >
        Hinzufügen
      </button>
    </div>
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center space-x-2 min-w-[184px] justify-end">
            ...
            <button
              ...
              class="p-2 bg-green-500 rounded hover:bg-green-400 transition transform hover:scale-105"
            >
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512.044 512.044" class="w-6 h-6 fill-black">
              <g>
                <g>
            ...
            <button
              @click="rmSpieler(row)"
              class="p-2 bg-red-500 rounded hover:bg-red-400 transition transform hover:scale-105"
            >
              ...
              >
                <path
                  d="M2,21h8a1,1,0,0,0,0-2H3.071A7.011,7.011,0,0,1,10,13a5.044,5.044,0,1,0-3.377-1.337A9.01,9.01,0,0,0,1,20,
                     1,1,0,0,0,2,21ZM10,5A3,3,0,1,1,7,8,3,3,0,0,1,10,5ZM23,16a1,1,0,0,1-1,1H15a1,1,0,0,1,0-2h7A1,1,0,0,1,23,16Z"
                />
              </svg>
            </button>
          </div>
          ...
          >
            <table class="text-sm">
              <tbody>
                ...
                <tr>
                  <td class="font-bold pr-2">Offene Kisten:</td>
                  <td>{{ row?.kiste }}</td>
                </tr>
              </tbody>
            </table>
...
async function addSpieler() {
  await $fetch("/api/kader-add-spieler", {
    method: "POST",
    body: { name: spielerInput.value },
  });
  kader.value = await $fetch("/api/kader-show");
  spielerInput.value = "";
}
...
async function rmSpieler(spieler: any) {
  await $fetch("/api/kader-rm-spieler", {
    method: "POST",
    body: { spieler },
  });
  kader.value = await $fetch("/api/kader-show");
...
async function trainingAddSpieler(spieler: any) {
  await $fetch("/api/training-add-spieler", {
    method: "POST",
    body: { spieler },
  });
  kader.value = await $fetch("/api/kader-show");
