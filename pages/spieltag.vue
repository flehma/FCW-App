<template>
    <div class="p-2">
        <h2 class="text-lg font-bold mt-4">Spieltag</h2>
...
<script setup lang="ts">
    ...
    const kader = ref<any>([]);
    kader.value = await $fetch('/api/spieltag-show')
    const restKader = ref<any>([]);
    restKader.value = await $fetch('/api/spieltag-show-rest-kader')

    const selectedSpieler = ref('');

    async function rmSpieler(spieler: any) {
        const res = await $fetch('/api/spieltag-rm-spieler', {
            method: "POST",
            body: {
                spieler: spieler
            }
