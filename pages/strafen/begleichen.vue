<template>
    <div class="p-2">
        <h2 class="text-lg font-bold mt-4">Strafen begleichen</h2>
            ...
            <div class="" v-for="row in kader?.kader" :key="row.id">
                <div class="p-3 rounded bg-slate-100 flex items-center justify-between">
                    ...
                    <div class="flex items-center justify-between">  
                        <div class="mr-2 p-2  w-[60px] font-semibold text-center rounded bg-blue-500 hover:bg-blue-400">
                            {{ row?.geld }}€<br>
                        </div>
                        <div>
                            <button @click="paid(row)" class="p-2 px-4 w-[60px] bg-red-500 text-white rounded hover:bg-red-400">
...
<script setup lang="ts">
    ...
    const kader = ref<any>([]);
    kader.value = await $fetch('/api/strafen-spieler-show')

    const spielerInput = ref('');
    let showDetails = ref<any>([]);
    showDetails.value = false;

    async function paid(spieler: any) {
        const res = await $fetch('/api/strafen-begleichen', {
            method: "POST",
            body: {
            spieler: spieler
            }
